    ...
    with tab:
        ...
        for i in range(7):
            ...
            for i, name in enumerate(staff_list):
                ...
                for j in range(48):
                    t = 7 + (j * 0.5)
                    c_type = "bg-closed" if not (biz_hours[0] <= t < biz_hours[1]) else ""
                    ...
                    body_html += f'<div class="cell-timeline {"hour-mark" if j%2==1 else ""} {c_type}"></div>'
                if i == 0:
...
target_date_str = st.sidebar.selectbox("対象の日付を選択", [f"{(monday_this_week + timedelta(days=i)).month}/{(monday_this_week + timedelta(days=i)).day}({weekdays_short[(monday_this_week + timedelta(days=i)).weekday()]})" for i in range(35)])
