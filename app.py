...
st.markdown(f"""
    ...
    .cell-header {{ background-color: #000; color: #fff; text-align: center; font-size: 8px; height: 22px; line-height: 22px; border-right: 1px solid #555; }}
    ...
    .vertical-merge-cell {{ grid-row: span {len(staff_list)}; border-right: 1px solid #000; border-bottom: 1px solid #000; display: flex; align-items: center; justify-content: center; text-align: center; font-size: 10px; font-weight: bold; background-color: #fff; }}
        ...
        for i in range(7):
            ...
            if df_sheet is not None:
                for idx, row in df_sheet.iterrows():
                    if disp.split('(')[0] in str(row[0]):
                        rec_val, live_val = (row[74], row[75]) if len(row) > 75 else ("", "")
                        break
